  ...
  function allCenter($x) {
    ...
    $x.find(".hcent").each(function () {
      ...
      var mat = $(this)
        .css("transform")
        .match(/(-?[0-9\.]+)/g);
      if (mat) var ty = parseInt(mat[5]) + "px";
      else var ty = "0";
    ...
    $x.find(".imginit").each(function () {
      ...
      if (thish > windh) {
        if (thish / windh > 1.3 || true) $(this).css("flex-direction", "row");
        else;
...
$(window).on("load", function () {
  setTimeout(function () {
    ...
    $("img:not(.png)").each(function () {
      $(this).css(
        "border-radius",
        parseInt($("#findme").css("width")) * 0.06925
      );
    });
  }, 200);
  ...
  tl2
    ...
    .fromTo(
      ...
      { background: "linear-gradient(to right bottom, #0F518F, #004227)" },
      "-=20"
    )
    ...
    .to(
      "#pinContainer",
      8,
      { background: "linear-gradient(to right bottom, #BB7700, #BB0000)" },
      "-=20"
    )
    // .fromTo(  ".naver",  1,  {background: "linear-gradient(to right bottom,#333,#222)"},  {background: "linear-gradient(to right bottom, #004060, #002035)"},  "-=4")
    // .to(  "#pt1",  1,  {scale: 1.2, textShadow: "1px 1px 20px #FFFFFF66"},  "-=1")
    ...
    .to(
      "#pinContainer",
      8,
      { background: "linear-gradient(to right bottom, #210101, #810101)" },
      "-=20"
    )
    ...
    .to(
      "#pinContainer",
      8,
      { background: "linear-gradient(to right bottom, #393939, #171717)" },
      "-=20"
    )
    ...
    .to(
      "#pinContainer",
      8,
      { background: "linear-gradient(to right bottom, #000, #061D33)" },
      "-=20"
    )
    .to("#project4 .pinit", 8, { opacity: 0, scale: 0.7 }, "+=8")
    .staggerFrom(
      "#project4 .imginit div",
      8,
      { scale: 0.9, opacity: 0 },
      2,
      "-=4"
